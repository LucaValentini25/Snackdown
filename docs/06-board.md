...
**Overall:** 84% — 41 done, 0 in progress, 0 blocked, 8 to do, 2 dropped, across 8 epics.
...
| [Verification — make the netcode claims checkable](#verification-make-the-netcode-claims-checkable) | 5 | To do | 3/4 |
...
|:---:|---|---|---|
| `[ ]` | The latency measurements in docs/05 predate the reconciler extraction | The figures there — median error 0.302 units at 150 ms and 20% loss — were recorded against Reconcile as it lived inside PredictedPlayer. vf-2 moved the decisions out and the branch-by-branch tests say they behave the same, but nothing has re-run the measurement, and a refactor of the headline mechanism is exactly the change whose effect a unit test can miss. The run is a documented manual procedure and takes minutes. | open |
