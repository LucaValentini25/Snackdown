...
|:---:|---|---|---|
| `[ ]` | Most of the lead to the authoritative position is still unattributed | Measured 2026-08-26 over Relay: 400ms of lead, a 3.0 unit gap to the ghost, and a server input queue sitting at 2-3. MaxQueueDepth is exonerated — it is 8 and the queue never approaches it, so lowering it would have cost jitter tolerance for nothing. The queue accounts for about 83ms; the remaining ~317ms is transit plus the lead NGO runs the client clock at, and is not attributed. The gap itself is exactly the round trip drawn, with no unexplained separation on top. What is left is the loopback control in docs/05, which separates the relay hop from the client lead. | open |
