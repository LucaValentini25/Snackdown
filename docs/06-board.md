...
**Overall:** 67% — 33 done, 0 in progress, 0 blocked, 16 to do, 2 dropped, across 8 epics.
...
| [Wardrobe — unique, changeable skins](#wardrobe-unique-changeable-skins) | 4 | To do | 2/7 |
...
| `[ ]` | Wire CharacterCount from CharacterCatalog.Count | a fifth catalog entry is selectable | Hardcoded 4 today, so a fifth skin is authored and silently unreachable. Held back from wr-1 on purpose: CharacterCatalog lives in Snackdown.Gameplay, which already depends on Snackdown.Connection, so approval cannot name it and this is an assembly decision rather than a wiring job. Same shape as the one D-014 settled for the roster |
