...
**Overall:** 63% — 31 done, 0 in progress, 0 blocked, 18 to do, 2 dropped, across 8 epics.
...
| [Wardrobe — unique, changeable skins](#wardrobe-unique-changeable-skins) | 4 | To do | 0/7 |
...
| | Task | Verified by | Notes |
|:---:|---|---|---|
| `[ ]` | Return to lobby comes back to the lobby, not to the menu | a host and a client finish a match, press Return to lobby, and both land in the lobby they were in — roster, ready state and join code intact — without reconnecting | Reported by Luca after playing. The Lobby scene is unloaded at Countdown and reloaded when the phase returns, so what comes back is a fresh MainMenuController whose OnEnable ends in ShowMenu() without asking whether a session is live. The connection is never lost; only the screen is. Two parts, and the second is why this is not a one-liner: OnEnable has to choose the screen from the live session, and the join code has to survive — _rawJoinTarget and the IConnectionProvider that produced it are fields of the controller and die with the scene, so even the right screen comes back with an empty code row and a Copy button with nothing to copy. Either the lobby stops being unloaded and is only hidden, which contradicts the one-owner rule for that scene, or the connection state moves out of the controller. First in this epic because a wardrobe is changed in the lobby, and a lobby you cannot return to makes every task below it untestable by hand |
